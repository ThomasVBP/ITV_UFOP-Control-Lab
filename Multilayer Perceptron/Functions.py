...
def network_dictionary(network_file):
    ...
    with open(network_file, 'r') as file:
        ...
        for line in file:
            ...
            elif line.startswith("Weight for Neuron"):
                parts = line.split(":")
                neuron = int(parts[0].split()[-1])
                weight = float(parts[1].strip())
                layer['Neurons'][-1]['Weights'][f'Weight for Neuron {neuron}'] = weight
            elif line.startswith("Bias for Neuron"):
                parts = line.split(":")
                neuron = int(parts[0].split()[-1])
                bias = float(parts[1].strip())
                layer['Biases'][f'Bias for Neuron {neuron}'] = bias
        if layer:  
            layers.append(layer)
                    ...
                    if j == x:
                        ...
                        for weight_key, weight_value in weight_neurons.items():
                            ...
                            if i == len(neural_network['Layers']):  
                                with open(destination_file, 'a') as file:
                                    if j == 0:
                                        file.write(f'N{i}{number_neurons} := (')
                                    else:
                                        file.write(f'{y} * {weight_value} + ')
                            else:
                                if number_neurons in used_neurons:
                                    with open(destination_file, 'r') as read_file:
                                        lines = read_file.readlines()
                                    with open(destination_file, 'w') as file:
                                        for line in lines:
                                            if line.startswith(f'N{i}{number_neurons} :='):
                                                new_line = f'{line.strip()} + {y} * {weight_value}\n'
                                                file.write(new_line)
                                            else:
                                                file.write(line)
                                else:
                                    with open(destination_file, 'a') as file:
                                        file.write(f'N{i}{number_neurons} := ({y} * {weight_value}\n')
                                    used_neurons.add(number_neurons)
                            key = f'N{i}{number_neurons} '
