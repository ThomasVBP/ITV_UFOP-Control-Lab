...
Within [decision_tree_converter](src/decision_tree_converter) and [multilayer_perceptron_converter](src/multilayer_perceptron_converter), there is a dataset collected from historical data of the circuit operation. The features are the average and standard deviation of the last 10 current data, and the output is the mass flow rate. Using the *conversion.py* file from the associated machine learning algorithm, one can create and convert both a Decision Tree and a Multilayer Perceptron to Structured Text code for this example.
